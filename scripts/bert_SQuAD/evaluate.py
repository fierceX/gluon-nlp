...
def predictions(dev_dataset,
    ...
    for feature, start_logit, end_logit in zip(dev_dataset, start_logits,
                                               end_logits):

        prelim_predictions = []

        start_indexes = _get_best_indexes(start_logit.tolist(), n_best_size)
        end_indexes = _get_best_indexes(end_logit.tolist(), n_best_size)
